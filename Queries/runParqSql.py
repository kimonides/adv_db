...
genres = spark.read.parquet("hdfs://master:9000/data/movie_genres.parquet")
...
def benchmark():
    ...
    for (i,q) in enumerate(queries):
        ...
        sys.stdout = open('./results_parquet_sql/query_%s'%i, 'w+') 
        startTime = time.time()
        res = spark.sql(q)
        res.show()
        endTime = time.time()
    ...
    sys.stdout = open('./results_parquet_sql/times', 'w+') 
